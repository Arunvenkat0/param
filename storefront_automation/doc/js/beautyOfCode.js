jQuery.beautyOfCode = {
    ...
    init: function(clipboardSwf, settings) {
        ...
        if (settings) jQuery.extend(jQuery.beautyOfCode.settings, settings);

        if (jQuery.beautyOfCode.isInitialized) return;

        // creates a map of each registered brush by alias
        jQuery.each(SyntaxHighlighter.brushes,
        function(i, brush) {
    ...
    beautifyAll: function() {
        jQuery("pre.code:has(code[class])").each(function(i, item) {
            ...
            if ($code.hasClass("boc-nogutter")) settings.noGutter = true;

            if ($code.hasClass("boc-nocontrols")) settings.addControls = false;

            if ($code.hasClass("boc-showcolumns")) settings.showColumns = true;

            if ($code.hasClass("boc-collapse")) settings.collapse = true;

            var firstLine = getOptionValue("boc-firstline", options, 1);
            if (firstLine) settings.firstLine = firstLine;
...
jQuery.fn.beautifyCode = function(language, settings) {
    ...
    this.each(function(i, item) {
        ...
        var settings = jQuery.extend({},
        	jQuery.beautyOfCode.settings, saveSettings);
        ...
        SyntaxHighlighter.defaults['wrap-lines'] = settings.wrapLines;
        ...
        if ($item.is("pre") && ($code = $item.children("code"))) $item.text($code.text());
